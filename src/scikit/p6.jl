...
for i in 1:length(topologies)
	...
	topology_parameters["max_epochs"] = 1000;
	topology_parameters["max_epochs_val"] = 30;
...
# train_idx,test_idx = holdOut(size(targets,1),0.2)
# train_inputs,train_targets = inputs[train_idx,:],targets[train_idx]
# test_inputs,test_targets = inputs[test_idx,:],targets[test_idx]

# model = DecisionTreeClassifier(max_depth=models_parameters[3]["max_depth"], random_state=1);
...
# matrix =confusionMatrix(test_outputs,test_targets,macro_strat)[8]
# display(classes)
# display(matrix)